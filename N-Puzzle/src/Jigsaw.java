...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
        ...
        while (!openList.isEmpty()) {
            ...
            Vector<JigsawNode> next = findFollowJNodes(currentJNode);
            for (JigsawNode j : next) {
                openList.addElement(j);
            }
        }
	...
	private void estimateValue(JigsawNode jNode) {
	    int sum = 0;
	    int[] cofficient = {10, 1, 20};

	    int s = 0; // 后续节点不正确的数码个数
	    int v = 0;
	    int d = 0;
	    int dimension = jNode.getDimension();
	    int[] map = jNode.getNodesState();
	    int index = 1;
	    
	    for (int i = 1; i <= dimension; i++) {
            for (int j = 1; j <= dimension; j++) {
	            if ((j < dimension) && (map[index]+1 != map[index+1])) { 
	                s++;
	            }
	            if ((i < dimension) && (map[index]+dimension != map[index+dimension])) { 
	                s++;
	            }
	            if (map[index] != index) {
                    v++;
                ...
                int b = map[index]-1;
                d += Math.abs((j-1) - (b % dimension)) + Math.abs((i-1) - (b / dimension));
                index++;
            }
	    }
	    sum = cofficient[0]*s + cofficient[1]*v + cofficient[2]*d;
